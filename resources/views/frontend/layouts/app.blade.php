...
<head>
    ...
    <meta name=twitter:image content=https://www.update.shipa1.com/img/car-tranportation.png>
...
<body>
    ...
    <div id="_71A63tRBiHHb3tRBnQfK_1" class="proofNotificationWrapper">
        <div class="_71A63x22iHHb3x22nQfK_1">
            <div>
                <div class="_71A62qKhiHHb2qKhnQfK_0">
                    <div class="_71A6iq4LiHHbiq4LnQfK_0">
                        <img id="car-image"
                            src="https://firebasestorage.googleapis.com/v0/b/proof-f6589.appspot.com/o/maps2%2F1c5e54f51c1b73102d32ae392dc30817.png.png?alt=media"
                            alt="Car Image">
                    </div>
                    <div class="_71A61iAyiHHb1iAynQfK_0">
                        <div class="_71A62XMSiHHb2XMSnQfK_0">
                            <span id="customer-name">Amber from Austin, Texas</span>
                        </div>
                        <div class="_71A66Nl7iHHb6Nl7nQfK_0">Just booked an order</div>
                        <div class="_71A62e55iHHb2e55nQfK_0" id="order-time">10 hours ago</div>
                    </div>
                </div>
            </div>
        </div>
     ...
     <script>
        $(document).ready(function () {
            ...
            const names = [
                ...
                "Charlotte from Nevada",
                "Ethan from Georgia",
                "Amelia from Michigan",
                "Alexander from Washington",
                "Mia from North Carolina",
                "Daniel from Colorado",
                "Harper from Pennsylvania",
                "Henry from Tennessee",
                "Evelyn from Virginia",
                "Lucas from Massachusetts",
                "Abigail from Indiana",
                "Matthew from Wisconsin",
                "Ella from Missouri",
                "Jackson from Minnesota",
                "Scarlett from Oregon",
                "David from Kentucky",
                "Lily from Oklahoma",
                ...
                "Nora from New Mexico",
                "Elijah from Mississippi",
                "Layla from Arkansas",
                "Nathan from Hawaii",
                "Hannah from South Dakota",
                "Dylan from North Dakota",
                "Madison from Montana",
                "Caleb from Idaho",
                "Lillian from Maine",
                "Joshua from Vermont",
                ...
                "Victoria from Connecticut",
                "Luke from New Hampshire",
                "Natalie from Washington D.C."
            ];

            // Function to generate a random time (minutes or hours ago)
            function randomTime() {
                const value = Math.floor(Math.random() * 12) + 1;
                const unit = Math.random() > 0.5 ? " hours ago" : " minutes ago";
                return value + unit;
            }

            // Function to update the notification content
            function updateNotification() {
                $("#car-image").attr("src", carImages[Math.floor(Math.random() * carImages.length)]);
                $("#customer-name").text(names[Math.floor(Math.random() * names.length)]);
                $("#order-time").text(randomTime());
            }
    ...
    <script>
        ...
        $(window).on('load', function() {
            ...
            $('#chat-widget').attr('src', "https://www.update.shipa1.com/chat_dashboard?user_id=0");
            $('#chat-widget-container').addClass('inactivee');
            $('#chat_with_us').addClass('activee');
            ...
            document.querySelectorAll(".validate-form").forEach(function(form) {
                function validateInput(input) {
                    ...
                    if (!input.value || (input.tagName === "SELECT" && input.value === "")) {
                        errorMessage.textContent = input.tagName === "SELECT" ?
                        ...
                        let validPrefixes = [
                            ...
                            '928', '327', '479',
                            '501', '870', '209', '213', '279', '310', '323', '341', '408', '415',
                            '424', '442', '510',
                            '530', '559', '562', '619', '626', '628', '650', '657', '661', '707',
                            '714', '747', '752',
                            '760', '805', '818', '820', '831', '858', '909', '916', '925', '949',
                            '951', '303', '719',
                            '720', '970', '203', '475', '860', '959', '302', '239', '305', '321',
                            '352', '386', '407',
                            '448', '561', '656', '727', '754', '772', '786', '813', '850', '863',
                            '904', '941', '954',
                            '229', '404', '470', '478', '678', '706', '762', '770', '912', '808',
                            '208', '986', '217',
                            '224', '309', '312', '331', '447', '464', '618', '630', '708', '773',
                            '779', '815', '847',
                            '872', '219', '260', '317', '463', '574', '765', '812', '930', '319',
                            '515', '563', '641',
                            '712', '316', '620', '785', '913', '270', '364', '502', '606', '859',
                            '225', '318', '337',
                            '504', '985', '207', '240', '301', '410', '443', '667', '339', '351',
                            '413', '508', '617',
                            '774', '781', '857', '978', '231', '248', '278', '313', '517', '586',
                            '616', '679', '734',
                            '810', '906', '947', '989', '218', '320', '507', '612', '651', '763',
                            '952', '228', '601',
                            '662', '769', '314', '417', '557', '573', '636', '660', '816', '406',
                            '308', '402', '531',
                            '702', '725', '775', '603', '201', '551', '609', '640', '732', '848',
                            '856', '862', '908',
                            '973', '505', '575', '212', '315', '332', '347', '516', '518', '585',
                            '607', '631', '646',
                            '680', '716', '718', '838', '845', '914', '917', '929', '934', '252',
                            '336', '472', '704',
                            '743', '828', '910', '919', '980', '984', '701', '216', '220', '234',
                            '283', '326', '330',
                            '380', '419', '440', '513', '567', '614', '740', '937', '405', '539',
                            '572', '580', '918',
                            '458', '503', '541', '971', '215', '223', '267', '272', '412', '445',
                            '484', '570', '610',
                            '717', '724', '814', '878', '401', '803', '839', '843', '854', '864',
                            '605', '423', '615',
                            '629', '731', '865', '901', '931', '210', '214', '254', '281', '325',
                            '346', '361', '409',
                            '430', '432', '469', '512', '682', '713', '726', '737', '806', '817',
                            '830', '832', '903',
                            '915', '936', '940', '956', '972', '979', '385', '435', '801', '802',
                            '276', '434', '540',
                            '571', '703', '757', '804', '206', '253', '360', '425', '509', '564',
                            '304', '681', '262',
                            '414', '534', '608', '715', '920', '307', '800', '833', '844', '855',
                            '866', '877', '888'
                        ];

                        if (!validPrefixes.includes(phonePrefix)) {
                            errorMessage.textContent =
                                "Phone number must start with a valid USA prefix.";
                            input.style.border = "2px solid red";
                            isValid = false;
                        } else {
                            errorMessage.textContent = "";
    ...
    <script>
        ...
        // Example: Optimizing an input field
        document.getElementById("searchBox").addEventListener("input", debounce((event) => {
