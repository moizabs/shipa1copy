                    ...
                    <div class="col-lg-12">
                        <div class="p-5">
                            <div class="text-center">
                                <h1 class="h4 text-gray-900 mb-4">{{ isset($portToPort) ? 'Edit' : 'Add New' }} Port to Port
                                </h1>
                            </div>
                            <form
                                action="{{ isset($portToPort) ? route('port-to-ports.update', $portToPort->id) : route('port-to-ports.store') }}"
                                method="POST">
                                @csrf
                                @if (isset($portToPort))
                                    @method('PUT')
                                ...
                                <div class="form-group">
                                    ...
                                    <input class="form-control" type="number" step="0.01" name="price"
                                        placeholder="Price" value="{{ old('price', $portToPort->price ?? '') }}"
                                        required />
                                </div>
                                <button type="submit" class="btn btn-primary">{{ isset($portToPort) ? 'Update' : 'Add' }}
                                    Port to Port</button>
                            </form>
                        </div>
                    </div>
